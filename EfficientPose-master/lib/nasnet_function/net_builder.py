...
PRIMITIVES = {
    ...
    'dil_k3_e6': lambda C_in, C_out, expansion, stride, **kwargs: DilBlock(
        C_in, C_out, 6, stride, kernel=3, padding=2, dilation=2, **kwargs
    ),
    'dil_k5_e6':lambda C_in, C_out, expansion, stride, **kwargs: DilBlock(
        C_in, C_out, 6, stride, kernel=5, padding=4, dilation=2, **kwargs
    ),
    # layer search se
...
class ConvTransBNRelu(nn.Sequential):
    def __init__(
        self,
        input_depth,
        output_depth,
        ...
        **kwargs
        ...
        self.add_module("deconv", op)
        
        if bn_type == "bn":
            #bn_op = BatchNorm2d(output_depth)
            bn_op = nn.BatchNorm2d(output_depth, affine=False, track_running_stats=False)
        elif bn_type == "gn":
...
class NetBuilder(object):
    ...
    def add_blocks(self, blocks):
        ...
        modules = OrderedDict()
        for block in blocks:
            # block: {'stage_idx': 0, 'block_idx': 0, 'block': [3, 24, 1, 2], 'block_op_type': 'ir_k5_e3'}
            ...
            nn_name = "x{}_{}".format(stage_idx, block_op_type)
            #nn_name = "x{}_{}".format(stage_idx, block_idx)
